...
class PLAYGROUND_511_API UParkourMovementComponent : public UActorComponent
{
	GENERATED_BODY()
...
Reubs:
#include Engine.h
FTimerHandle YourTimerHandleName
bool bValueToBeChanged
void FunctionToCallWhenTimerIsDone

GetWorld()->GetTimerManger().SetTimer(YourTimerHandleName, this, SomeTimeValue, Loop?)


Dominik Frey:
#include TimerManager.h (Header)
#include Engine/World.h (C++)
void DelaySomething();
FTimerHandle YourTimerHandleName (inside the function)

GetWorld()->GetTimerManager().SetTimer(YourTimerHandleName, this, FunctionToBeCalled, DelayTime, Loop?)
If YourTimerHandleName.IsValid() { GetWorld()->GetTimerManager().ClearTimer(YourTimerHandleName) } (Only if the timer is looping)

If(YourTimerHandleName.IsValid())
	If(GetWorld()->GetTimerManager().IsTimerPaused(YourTimerManager)
		GetWorld()->GetTimerManager().UnPauseTimer(YourTimerManager
If(YourTimerHandleName.IsValid())
	!If(GetWorld()->GetTimerManager().IsTimerPaused(YourTimerManager)
		GetWorld()->GetTimerManager().PauseTimer(YourTimerManager)

if (GEngine) { GEngine->AddOnScreenDebugMessage(-1, 2.0f, FColor::Yellow, TEXT("Parkour Movement Called")); }
//UE_LOG(LogTemp, Warning, TEXT("NewMode: %s"), *UEnum::GetValueAsString(NewMode));
*/